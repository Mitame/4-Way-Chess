def variables(): #Sets Variables
    ...
    #Pygame stuff
    global screenSize, bgcolour,debug
...
def startpiecesetup(varsonly = False):
    global detailPiece,detailIcon,detailColour,detailCoord,detailDirection,detailImagePath,detailMoved,detailStatus,detailTeam,detailImage,saveturn,livingteams,turn,team1,team2
    ...
    if varsonly == False:
        ...
        for piece in detailPiece:
            ...
            path = os.path.join("Images",detailColour[detailPiece.index(piece)],(detailIcon[detailPiece.index(piece)]+".png"))
            detailImage.append(pygame.transform.rotozoom(pygame.image.load(path),detailDirection[detailPiece.index(piece)],sizeIconDesired/sizeIcon))
        for let in ["A","B"]: #red
...
def displayrefresh(wait=False):
    ...
    while run != len(detailIcon):
        if detailStatus[run] == "active":
            screen.blit(detailImage[run],pygame.Rect.move(detailImage[run].get_rect(),chesstopixel(detailCoord[run])))
            if wait == True:
    ...
    if piece[1].isnumeric():
        if piece in detailCoord:
            if to in detailCoord:
                piecekiller(to,detailColour[detailCoord.index(to)])
...
def readinputs(team=None,refresh=0,gameinprogress = True):
    global run,start,legals,clicks,debug
    clicks = 0
        ...
        if event.type == pygame.QUIT:
            ...
            if event.pos <= (800,800) and gameinprogress == True:
                if run == 0:
                    ...
                    if end in legals and not start == end and debug == 0:
                        piecemove(start,end)
                        run = 0
                        if refresh == 1:
                            displayrefresh()
                        return "done"
                    elif debug == 1 and start != end:
                        piecemove(start,end)
                        run = 0
                        if refresh == 1:
                            displayrefresh()
            ...
            if event.key == 32 and event.mod == 64:
                if debug == 1:
                    debug = 0
                    print("Debug Off")
                elif debug == 0:
    ...
    if False:
        ...
        while 1:
            for turn in livingteams:
                if not team1[0] in livingteams and not team1[1] in livingteams:
                    print("Team 2 ("+team2[0],"and",team2[1]+") has won!")
                    sys.exit()
                elif not team2[0] in livingteams and not team2[1] in livingteams:
                    print("Team 1 ("+team1[0],"and",team1[1]+") has won!")
                    sys.exit()
